...
class EditTaskPage extends Component {
  ...
  setDefaultData() {
    if (this.props.nav.routes[this.props.nav.index].params && this.props.nav.routes[this.props.nav.index].params.taskid) {
      let task = this.props.user.createdtasks.find((item) => {return (item.id == this.props.nav.routes[this.props.nav.index].params.taskid)});

      if (task) {
        let end_date_full = task.end_at ? new Date(task.end_at*1000) : undefined;
        let end_date = end_date_full ? ("0" + end_date_full.getUTCDate()).slice(-2) + '.' + ("0" + (end_date_full.getUTCMonth()+1)).slice(-2) + "." + end_date_full.getUTCFullYear() : undefined;
        let end_time = end_date_full ? ("0" + end_date_full.getUTCHours()).slice(-2) + ":" + ("0" + end_date_full.getUTCMinutes()).slice(-2) : undefined
        console.warn(end_date)
        console.warn(end_time)
        let start_date_full = task.start_at ? new Date(task.start_at*1000) : undefined;
        let start_date = start_date_full ? ("0" + start_date_full.getUTCDate()).slice(-2) + '.' + ("0" + (start_date_full.getUTCMonth()+1)).slice(-2) + "." + start_date_full.getUTCFullYear() : undefined;
        let start_time = start_date_full ? ("0" + start_date_full.getUTCHours()).slice(-2) + ":" + ("0" + start_date_full.getUTCMinutes()).slice(-2) : undefined
        console.warn(start_date)
        ...
        this.state = {
          ...
          task: {
            ...
            distance: task.distance ? task.distance : 20 ,
            address: task.address,
            lat: task.lat,
            lng: task.lng,
            payment_type: task.payment_type,
            only_pro: task.only_pro ? true : false,
            only_business: task.only_business ? true : false,
            for_all: !task.only_pro && !task.only_business ? true : false,
            paid: task.paid ? true : false,
            ...
            enableAddress: task.address && task.lat && task.lng ? true : false,
            taskid: task.id,
          },
        }
      }
      console.warn(JSON.stringify(this.props.nav.routes[this.props.nav.index], 0 , 2))
  ...
  componentDidMount() {
    ...
    this.formErrorListener = EventRegister.addEventListener('ERROR_EVENT', (data) => {
      this.errors.alertWithType('warn', 'Ошибка при редактировании задачи', data);
    });

    this.redirectSuccessListener = EventRegister.addEventListener('REDIRECT_TO_MY_TASKS', (data) => {
      this.props.navigateToMyTasks();
    });
...
/*    this.setState({
  ...
  componentWillUnmount() {
    ...
    EventRegister.removeEventListener(this.redirectSuccessListener);
  };
      ...
      <ScrollView 
        ...
        <View style={styles.details}>
          <FormInput 
            ...
            placeholder="The Endgame is Onmipotence: бочка и бас" />
    ...
    let step4 = 
      <ScrollView 
        ...
        <View style={styles.details}>
          ...
          <TouchableHighlight underlayColor="transparent" onPress={() => this.editTask()}>
            <LinearGradient style={[buttonsStyles.buttonWrapper, buttonsStyles.round]} colors={['#3dccc6', '#31a3b7']}>
              <View>
                <View style={[buttonsStyles.buttonBig]}>
                  <Text style={buttonsStyles.buttonBigText}>Сохранить задачу</Text>
                </View>
              </View>
            </LinearGradient>
          </TouchableHighlight>
  ...
  editTask() {
    this.props.editOldTask(this.state.task);
  }
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
      ...
      editOldTask: (params) => editOldTask(params),
